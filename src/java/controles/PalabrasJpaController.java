...
public class PalabrasJpaController implements Serializable {
    ...
    public List<Palabras> buscarPorTipo(int id) {
        EntityManager em = getEntityManager();
        Query q=em.createQuery("select p From Palabras p JOIN p.tipo t where t.idtipos= :tipo");
        q.setParameter("tipo", id);
        List<Palabras> tipos=q.getResultList();
        return tipos;
    }
    ...
    public Palabras findPalabras(Integer id) {
     ...
     public Palabras findPalabrasNombre(String palabras) {
        EntityManager em = getEntityManager();
        Query q=em.createQuery("select p From Palabras p where p.palabra = :palabra");
        q.setParameter("palabra", palabras);
        Palabras palabra= (Palabras)q.getSingleResult();
        return palabra;
    }
     
     public List<Palabras> getNivel(int tipo,int nivel) {
        EntityManager em = getEntityManager();
        Query q=em.createQuery("select p From Palabras p JOIN"
                + " p.tipo t where p.nivel = :nivel and t.idtipos= :tipo");
